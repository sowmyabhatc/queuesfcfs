#include <assert.h>
#include <stddef.h>
#include "scheduler.h"
#include <stdio.h>

Process process_new (uint32_t pid, uint32_t at, uint32_t et)
{
    Process p = { pid, at, et, 0, 0};

    return p;
}


Queue  queue_new(uint32_t size)
{
	size = (size > 0 && size < MAX_QUEUE_LEN) ? size : MAX_QUEUE_LEN;

    Queue q = {size, /*count*/ 0, /*head*/0, /*tail*/0, /*Process*/{0} };

    return q;
}

Queue* queue_add(Queue *q, Process *p, QueueResult *result)
{
	assert(q != NULL && q->count <= q->size);

    if (q->count < q->size) {
        q->q[q->tail] = *p;
        q->tail = (q->tail + 1) % q->size;
        ++q->count;
        result->status = QUEUE_OK;
    } else {
        result->status = QUEUE_FULL;
    }

    assert(result->status == QUEUE_OK || q->count == q->size);
    return q;
}

Queue* queue_remove(Queue *q, QueueResult *result)
{
	assert(q != NULL && q->count <= q->size);

    if (q->count > 0) {
        result->pid = q->q[q->head].pid;
        result->wt  = q->q[q->head].wait_time;
        result->tat = q->q[q->head].tat_time;
        q->head = (q->head + 1) % q->size;
        --q->count;
        result->status = QUEUE_OK;
    } else {
        result->status = QUEUE_EMPTY;
    }

    assert(q->count < q->size);
    assert(result->status == QUEUE_OK || q->count == 0);

    return q;
}

uint32_t queue_full(Queue *q)
{
	assert(q != NULL && q->count <= q->size);
    return (q->count == q->size);
}

uint32_t queue_empty(Queue *q)
{
    return (q->count == 0);
}

static void _schedule_(Queue *q, QueueResult *res)
{
    static uint32_t cur_time = 0;
    uint32_t wt_time, t_time;

    q->q[q->head].wait_time = cur_time - q->q[q->head].a_time;
    cur_time += q->q[q->head].e_time;
    q->q[q->head].tat_time  = cur_time - q->q[q->head].a_time;


}


void fcfs_algorithm(Queue *q, QueueResult *res)
{
    //while (q->count > 0)
    //{
        _schedule_(q, res);
        queue_remove(q,res);
        //printf ("pid: %d\twait time: %d\tTAT: %d\n",res->pid,res->wt,res->tat);
    //}

}
